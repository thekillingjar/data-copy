...
TEST_F(ThreadPoolSTest, init_success) {
...
TEST_F(ThreadPoolSTest, count_down_latch_test) {
  FlowFuncThreadPool thread_pool("udf_thread_");
  std::atomic<uint32_t> count(0);
  auto func = [&thread_pool, &count](uint32_t thead_idx) {
    ++count;
    if (thead_idx % 2 == 0) {
      thread_pool.ThreadReady(thead_idx);
    } else {
      thread_pool.ThreadAbnormal(thead_idx);
    }
  };
  int32_t ret = thread_pool.Init(func, 2);
  EXPECT_EQ(ret, FLOW_FUNC_SUCCESS);
  thread_pool.WaitAllThreadReady();
  thread_pool.WaitForStop();
  EXPECT_EQ(count.load(), 2);
}

TEST_F(ThreadPoolSTest, device_init_success) {
...
TEST_F(ThreadPoolSTest, device_init_failed) {
  GlobalConfig::Instance().SetRunOnAiCpu(true);
...
drvError_t halGetDeviceInfoBitmapNotMatch(uint32_t devId, int32_t moduleType, int32_t infoType, int64_t *value) {
  switch (moduleType) {
...
TEST_F(ThreadPoolSTest, device_aicpu_notmatch) {
  GlobalConfig::Instance().SetRunOnAiCpu(true);
...
drvError_t halGetDeviceInfoAicpuNotExists(uint32_t devId, int32_t moduleType, int32_t infoType, int64_t *value) {
  switch (moduleType) {
    case MODULE_TYPE_AICPU:
      if (infoType == INFO_TYPE_OS_SCHED) {
        *value = 0L;
...
TEST_F(ThreadPoolSTest, device_aicpu_not_exists) {
  GlobalConfig::Instance().SetRunOnAiCpu(true);
...
TEST_F(ThreadPoolSTest, call_system_failed) {
  FlowFuncThreadPool thread_pool("udf_thread_");
