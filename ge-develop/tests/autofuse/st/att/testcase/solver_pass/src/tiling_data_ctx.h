...
BEGIN_TILING_DATA_DEF(TilingData)
  ...
  // 含义:属性参数定义
    TILING_DATA_FIELD_DEF(uint32_t, additional_output)
  ...
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q11)
  // 含义：单核输出的gm大小
  ...
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, BUF10)
  // 含义：输出的gm大小
  ...
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q23)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q12)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, BUF22)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q20)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q15)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q13)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q14)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q18)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q16)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q19)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q24)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q25)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q31)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q26)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q27)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q30)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q28)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q32)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q33)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q34)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q35)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q40)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q36)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q37)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q39)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q41)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q42)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q52)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q43)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q50)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q44)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q45)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q46)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q53)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q48)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q49)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q51)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q54)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q55)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q56)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q62)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q61)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q57)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q58)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q60)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q63)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q64)
  // 含义：Tbuf或者Queue使用的内存大小
  // 约束：定义了Tbuf或者Queue则会生成该参数
  // 计算公式：若该内存被多个算子使用，不考虑内存复用，则大小为各个算子的总和，考虑内存复用则为峰值时刻内存
    TILING_DATA_FIELD_DEF(uint32_t, Q65)
...
END_TILING_DATA_DEF
REGISTER_TILING_DATA_CLASS(AddLayerNorm, TilingData)
