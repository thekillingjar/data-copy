...
namespace ge{
namespace ascir{
extern std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcLog2TmpSizeV2(const ge::AscNode &node);
...
TEST_F(CalcLog2TmpSizeV2Test, CalcLog2TmpSize_ShouldReturnCorrectSize_WhenInputsIsFLOAT)
{
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::shared_ptr<ge::AscNode> node = graph.FindNode("log2");
    node->inputs[0].attr.vectorized_strides = {s1, Symbol(1)};
    std::vector<std::unique_ptr<ge::TmpBufDesc>> result = CalcLog2TmpSizeV2(*node);
