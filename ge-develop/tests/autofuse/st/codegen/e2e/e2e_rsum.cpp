...
void LoadRsumStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType data_type) {
  ...
  *rsum.y.strides = {One, Zero};
  rsum.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr(), 0}, {{ge::Symbol(8192), 0}, MemAttr(), 1}};
...
void LoadRsumInvalidAxisStore_BeforeAutofuse(ge::AscGraph &graph, ge::DataType data_type) {
  ...
  *load.y.strides = {s1 * s2 * s3, s2 * s3, s3, One};
  ...
  ge::ascir_op::Sum rsum("rsum");
  graph.AddNode(rsum);
  rsum.attr.tmp_buffers = {{{ge::Symbol(8192), -1}, ge::MemAttr(), 0}, {{ge::Symbol(8192), 0}, ge::MemAttr(), 1}};
  rsum.x = load.y;
  rsum.attr.sched.axis = {z0.id, z1.id, z2.id, z3.id};
  *rsum.y.axis = {z0.id, z1.id, z2.id, z3.id};
