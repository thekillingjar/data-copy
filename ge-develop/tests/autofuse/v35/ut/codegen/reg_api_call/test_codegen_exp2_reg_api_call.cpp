...
TEST(CodegenKernel, UnaryApiTmpV2Exp2Call) {
  ...
  *exp2_op.y.strides = {s1, One};
  ...
  exp2->attr.api.compute_type = ge::ComputeType::kComputeElewise;
  exp2->attr.api.type = ge::ApiType::kAPITypeCompute;
  exp2->attr.api.unit = ge::ComputeUnit::kUnitVector;
  exp2->attr.sched.loop_axis = z0.id;
  exp2->attr.tmp_buffers = {{{ge::Symbol(8192), -1}, MemAttr(), 0}};
  exp2->outputs[0].attr.vectorized_axis = {z1.id};
  exp2->outputs[0].attr.vectorized_strides = {One};
  exp2->outputs[0].attr.dtype = ge::DT_FLOAT;
  exp2->outputs[0].attr.mem.position = ge::Position::kPositionVecOut;
  exp2->outputs[0].attr.mem.tensor_id = 1;
  exp2->outputs[0].attr.mem.alloc_type = ge::AllocType::kAllocTypeQueue;
  exp2->outputs[0].attr.que.id = 2;
  exp2->outputs[0].attr.opt.merge_scope = ge::kIdNone;
