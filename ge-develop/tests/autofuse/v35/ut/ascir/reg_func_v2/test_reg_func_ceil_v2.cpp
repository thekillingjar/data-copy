...
namespace ge{
namespace ascir{
extern std::vector<std::unique_ptr<ge::TmpBufDesc>> CalcCeilTmpSizeV2(const ge::AscNode &node);
...
TEST_F(CalcCeilTmpSizeV2Test, CalcCeilTmpSize_ShouldReturnCorrectSize_WhenInputsIsFLOAT)
{
    ...
    *y.y.strides = {s1, Symbol(1)};
    ...
    std::shared_ptr<ge::AscNode> node = graph.FindNode("ceil");
    node->inputs[0].attr.vectorized_strides = {s1, Symbol(1)};
    std::vector<std::unique_ptr<ge::TmpBufDesc>> result = CalcCeilTmpSizeV2(*node);
