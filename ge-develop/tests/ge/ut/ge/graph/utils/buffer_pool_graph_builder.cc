...
ComputeGraphPtr BufferPoolGraphBuilder::BuildNormalGraph() {
  ...
  auto w1 = builder.AddNode("w1", DATA, 0, 1);
  auto w2 = builder.AddNode("w2", DATA, 0, 1);
  auto w3 = builder.AddNode("w3", DATA, 0, 1);
  auto w4 = builder.AddNode("w4", DATA, 0, 1);
  auto w5 = builder.AddNode("w5", DATA, 0, 1);
 ...
 *
 *      |_____p4_____|_____p5____|
 */
ComputeGraphPtr BufferPoolGraphBuilder::BuildMultiPrefetchGraph() {
  auto graph_builder = InnerGraphBuilder(graph_name_);
  auto w1 = graph_builder.AddNode("w1", VARIABLE, 0, 1);
  auto w2 = graph_builder.AddNode("w2", VARIABLE, 0, 1);
  auto w3 = graph_builder.AddNode("w3", VARIABLE, 0, 1);

  const int64_t buffer_pool_id = 0;
  const int64_t buffer_pool_size = 5600;

  auto prefetch1 = graph_builder.AddNode("prefetch1", HCOMALLGATHER, 1, 1);
  SetPrefetchNodeInfo(prefetch1, buffer_pool_id, buffer_pool_size, {500});
  auto prefetch2 = graph_builder.AddNode("prefetch2", HCOMALLGATHER, 1, 1);
  SetPrefetchNodeInfo(prefetch2, buffer_pool_id, buffer_pool_size, {500});
  auto prefetch3 = graph_builder.AddNode("prefetch3", HCOMALLGATHER, 1, 1);
  SetPrefetchNodeInfo(prefetch3, buffer_pool_id, buffer_pool_size, {500});
  auto prefetch4 = graph_builder.AddNode("prefetch4", HCOMALLGATHER, 1, 1);
  SetPrefetchNodeInfo(prefetch4, buffer_pool_id, buffer_pool_size, {1024});
  auto prefetch5 = graph_builder.AddNode("prefetch5", HCOMALLGATHER, 1, 1);
  SetPrefetchNodeInfo(prefetch5, buffer_pool_id, buffer_pool_size, {1024});
  auto prefetch6 = graph_builder.AddNode("prefetch6", HCOMALLGATHER, 1, 1);
  SetPrefetchNodeInfo(prefetch6, buffer_pool_id, buffer_pool_size, {1024});

  auto add1 = graph_builder.AddNode("add1", ADD, 2, 1);
  auto add2 = graph_builder.AddNode("add2", ADD, 2, 1);
  auto add3 = graph_builder.AddNode("add3", ADD, 2, 1);
  auto const1 = graph_builder.AddNode("const1", CONSTANTOP, 0, 1);
  auto net_output = graph_builder.AddNode("net_output", NETOUTPUT, 1, 0);

  graph_builder.AddDataEdge(w1, 0, prefetch1, 0);
  graph_builder.AddDataEdge(prefetch1, 0, prefetch2, 0);
  graph_builder.AddDataEdge(w2, 0, prefetch3, 0);
  graph_builder.AddDataEdge(prefetch3, 0, prefetch4, 0);
  graph_builder.AddDataEdge(w3, 0, prefetch5, 0);
  graph_builder.AddDataEdge(prefetch5, 0, prefetch6, 0);

  graph_builder.AddDataEdge(const1, 0, add1, 0);
  graph_builder.AddDataEdge(prefetch2, 0, add1, 1);

  graph_builder.AddDataEdge(add1, 0, add2, 0);
  graph_builder.AddDataEdge(prefetch4, 0, add2, 1);

  graph_builder.AddDataEdge(add2, 0, add3, 0);
  graph_builder.AddDataEdge(prefetch6, 0, add3, 1);

  graph_builder.AddDataEdge(add3, 0, net_output, 0);

  auto compute_graph = graph_builder.GetGraph();
  return compute_graph;
}
}  // namespace ut
}  // namespace ge
