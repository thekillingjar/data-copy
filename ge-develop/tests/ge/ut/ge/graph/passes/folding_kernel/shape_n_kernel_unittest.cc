...
#include "host_kernels/array_ops/shape_n_kernel.h"
...
class UtestGraphPassesFoldingKernelShapeNkernel : public testing::Test {
 protected:
  void SetUp() { init(); }
 ...
 private:
  void init() {
    ...
    op_desc_ptr_ = std::make_shared<OpDesc>("ShapeN", SHAPEN);
 ...
 protected:
  ...
  NodePtr init_node(ComputeGraphPtr graph) {
    // middle
    OpDescPtr op_def = std::make_shared<OpDesc>("op_def", SHAPEN);
...
TEST_F(UtestGraphPassesFoldingKernelShapeNkernel, SizeOptimizerSuccess) {
  ...
  if (out_weights.size() > 1) {
    ...
    EXPECT_EQ(21120, size);
  }
}

TEST_F(UtestGraphPassesFoldingKernelShapeNkernel, ShapeNDataInt32) {
  ...
  GeTensorDesc tensor_desc_out_0(GeShape(), FORMAT_NCHW, DT_INT32);
  op_desc_ptr_->AddOutputDesc(tensor_desc_out_0);

  GeTensorDesc tensor_desc_out_1(GeShape(), FORMAT_NCHW, DT_INT32);
  op_desc_ptr_->AddOutputDesc(tensor_desc_out_1);

  std::vector<GeTensorPtr> outputs;
  Status status = kernel_->Compute(nullptr, outputs);
  EXPECT_EQ(ge::FAILED, status);
  status = kernel_->Compute(node_, outputs);
  EXPECT_EQ(ge::SUCCESS, status);
  if (status == ge::SUCCESS) {
    EXPECT_EQ(outputs[1]->GetTensorDesc().GetDataType(), DT_INT32);
    EXPECT_EQ(outputs[1]->GetTensorDesc().GetShape().GetDimNum(), 1);
    EXPECT_EQ(outputs[1]->GetData().size(), sizeof(int32_t) * dims_vec_1.size());
    EXPECT_EQ(*(int32_t *)(outputs[1]->GetData().data()), 3);
  }
}

TEST_F(UtestGraphPassesFoldingKernelShapeNkernel, ShapeNDataInt64) {
  vector<int64_t> dims_vec_0 = {8, 2};
  GeTensorDesc tensor_desc_0(GeShape(dims_vec_0), FORMAT_NCHW, DT_FLOAT);
...
TEST_F(UtestGraphPassesFoldingKernelShapeNkernel, ShapeN2In1Out) {
  vector<int64_t> dims_vec_0 = {8, 2};
  GeTensorDesc tensor_desc_0(GeShape(dims_vec_0), FORMAT_NCHW, DT_FLOAT);
  TensorUtils::SetRealDimCnt(tensor_desc_0, dims_vec_0.size());
  op_desc_ptr_->AddInputDesc(tensor_desc_0);
