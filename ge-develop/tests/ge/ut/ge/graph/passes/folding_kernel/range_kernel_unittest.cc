...
#include "host_kernels/selection_ops/range_kernel.h"
...
TEST_F(UtestGraphPassesFoldingKernelRangeKernel, Int64Success) {
  OpDescPtr op_desc_ptr = std::make_shared<OpDesc>("Range", RANGE);
  AttrUtils::SetInt(op_desc_ptr, ATTR_NAME_T, (int64_t)DT_INT64);

  int64_t start = 1, limit = 20, delta = 2;

  vector<int64_t> dims_vec_0;
  vector<int64_t> data_vec_0 = {start};
  ...
  vector<int64_t> dims_vec_1;
  vector<int64_t> data_vec_1 = {limit};
  ...
  vector<int64_t> dims_vec_2;
  vector<int64_t> data_vec_2 = {delta};
  ...
  int64_t *val = (int64_t *)outputs[0]->GetData().data();
  for (int64_t i = 0; i < outputs[0]->GetTensorDesc().GetShape().GetDim(0); ++i) {
    EXPECT_EQ(val[i], start + delta * i);
  }
}

TEST_F(UtestGraphPassesFoldingKernelRangeKernel, DoubleSuccess) {
  OpDescPtr op_desc_ptr = std::make_shared<OpDesc>("Range", RANGE);
  AttrUtils::SetInt(op_desc_ptr, ATTR_NAME_T, (int64_t)DT_DOUBLE);

  double start = 0.0, limit = 10.0, delta = 1.0;
  ...
  double *val = (double *)outputs[0]->GetData().data();
  for (int64_t i = 0; i < outputs[0]->GetTensorDesc().GetShape().GetDim(0); ++i) {
    EXPECT_EQ(val[i], start + delta * (double)i);
  }
}
