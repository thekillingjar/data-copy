...
class UtestPlaceholderWithDefaultPass : public Test {
 protected:
...
TEST_F(UtestPlaceholderWithDefaultPass, succ) {
  ComputeGraphPtr graph = std::make_shared<ComputeGraph>("test");
  NodePtr node = AddNode(graph, "PlaceholderWithDefault", PLACEHOLDERWITHDEFAULT);
  ...
  PlaceholderWithDefaultPass pass;
  Status status = pass.Run(node);
  EXPECT_EQ(status, SUCCESS);
  NodePtr found_node = graph->FindNode("PlaceholderWithDefault");
