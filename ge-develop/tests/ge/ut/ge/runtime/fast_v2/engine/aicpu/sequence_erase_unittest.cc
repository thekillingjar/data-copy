...
namespace gert {
using namespace bg;
class SequenceEraseUT : public BgTestAutoCreateFrame {};

TEST_F(SequenceEraseUT, success) {
  DEF_GRAPH(g1) {
    CHAIN(NODE("data1", "Data")->NODE("SequenceEraseNode", "SequenceErase")->NODE("NetOutput", "NetOutput"));
    CHAIN(NODE("data2", "Data")->EDGE(0, 1)->NODE("SequenceEraseNode", "SequenceErase"));
  };
...
auto root_model = GeModelBuilder(graph).BuildGeRootModel();
  ...
  LowerInput input = {{data1_ret.out_shapes[0], data2_ret.out_shapes[0]},
                      ...
                      &global_data};
  auto sequence_ret = LoweringSequenceErase(graph->FindNode("SequenceEraseNode"), input);
  ASSERT_TRUE(sequence_ret.result.IsSuccess());
  ASSERT_EQ(sequence_ret.out_shapes.size(), 1);
  ASSERT_EQ(sequence_ret.out_addrs.size(), 1);
}

TEST_F(SequenceEraseUT, failed) {
  DEF_GRAPH(g1) {
    CHAIN(NODE("SequenceEraseNode", "SequenceErase")->NODE("NetOutput", "NetOutput"));
  };
