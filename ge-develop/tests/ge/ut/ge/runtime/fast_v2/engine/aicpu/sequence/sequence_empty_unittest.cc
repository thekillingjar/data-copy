...
namespace gert {
using namespace bg;
class SequenceEmptyUT : public BgTestAutoCreate3StageFrame {};
TEST_F(SequenceEmptyUT, success) {
  DEF_GRAPH(g1) {
    CHAIN(NODE("data1", "Data")->NODE("SequenceEmptyNode", "SequenceEmpty")->NODE("NetOutput", "NetOutput"));
    CHAIN(NODE("data2", "Data")->EDGE(0, 1)->NODE("SequenceEmptyNode", "SequenceEmpty"));
  };

  auto graph = ge::ToComputeGraph(g1);
  auto split_to_sequence_op_desc = graph->FindNode("SequenceEmptyNode")->GetOpDesc();
auto root_model = GeModelBuilder(graph).BuildGeRootModel();
  ...
  LowerInput input = {{data1_ret.out_shapes[0], data2_ret.out_shapes[0]},
                          ...
                          &global_data};
  auto sequence_ret = LoweringSequenceEmpty(graph->FindNode("SequenceEmptyNode"), input);
  ASSERT_TRUE(sequence_ret.result.IsSuccess());
  ASSERT_EQ(sequence_ret.out_shapes.size(), 1);
  ASSERT_EQ(sequence_ret.out_addrs.size(), 1);
}

TEST_F(SequenceEmptyUT, failed) {
  DEF_GRAPH(g1) {
    CHAIN(NODE("SequenceEmptyNode", "SequenceEmpty")->NODE("NetOutput", "NetOutput"));
  };
