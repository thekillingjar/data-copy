...
namespace gert {
using namespace bg;
class SequenceInsertUT : public BgTestAutoCreateFrame {};

TEST_F(SequenceInsertUT, success) {
  DEF_GRAPH(g1) {
    CHAIN(NODE("data1", "Data")->NODE("SequenceInsertNode", "SequenceInsert")->NODE("NetOutput", "NetOutput"));
    CHAIN(NODE("data2", "Data")->EDGE(0, 1)->NODE("SequenceInsertNode", "SequenceInsert"));
  };

  InitTestFrames();
  auto graph = ge::ToComputeGraph(g1);
  auto split_to_sequence_op_desc = graph->FindNode("SequenceInsertNode")->GetOpDesc();
...
TEST_F(SequenceInsertUT, failed) {
  ...
  LowerInput input = {{data1_ret.out_shapes[0]}, {data1_ret.out_addrs[0]}, &global_data};
  auto sequence_ret = LoweringSequenceInsert(graph->FindNode("SequenceInsertNode"), input);
