...
namespace gert {
using namespace bg;
class SequenceConstructUT : public BgTestAutoCreateFrame {};

TEST_F(SequenceConstructUT, success) {
  DEF_GRAPH(g1) {
    CHAIN(NODE("data1", "Data")->NODE("SequenceConstructNode", "SequenceConstruct")->NODE("NetOutput", "NetOutput"));
    CHAIN(NODE("data2", "Data")->EDGE(0, 1)->NODE("SequenceConstructNode", "SequenceConstruct"));
  };

  InitTestFrames();
  auto graph = ge::ToComputeGraph(g1);
  auto split_to_sequence_op_desc = graph->FindNode("SequenceConstructNode")->GetOpDesc();
auto root_model = GeModelBuilder(graph).BuildGeRootModel();
  ...
  LowerInput input = {{data1_ret.out_shapes[0], data2_ret.out_shapes[0]},
                          ...
                          &global_data};
  auto sequence_ret = LoweringSequenceConstruct(graph->FindNode("SequenceConstructNode"), input);
  ASSERT_TRUE(sequence_ret.result.IsSuccess());
  ASSERT_EQ(sequence_ret.out_shapes.size(), 1);
  ASSERT_EQ(sequence_ret.out_addrs.size(), 1);
}

TEST_F(SequenceConstructUT, failed) {
  DEF_GRAPH(g1) {
    CHAIN(NODE("SequenceConstructNode", "SequenceConstruct")->NODE("NetOutput", "NetOutput"));
  };
