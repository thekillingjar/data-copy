...
class FFTSPlusConfigurationUTEST : public testing::Test {
 protected:
  void SetUp() {
  }
...
TEST_F(FFTSPlusConfigurationUTEST, Initialize_failed){
...
TEST_F(FFTSPlusConfigurationUTEST, Finalize_success1) {
  Configuration &config = Configuration::Instance();
  config.is_init_ = false;
...
TEST_F(FFTSPlusConfigurationUTEST, Finalize_success2) {
  Configuration &config = Configuration::Instance();
...
TEST_F(FFTSPlusConfigurationUTEST, GetFftsOptimizeEnableByEnv_on) {
  Configuration &config = Configuration::Instance();
  config.runmode_ = "";
...
TEST_F(FFTSPlusConfigurationUTEST, GetFftsOptimizeEnableByEnv_off) {
  Configuration &config = Configuration::Instance();
  config.runmode_ = "";
...
TEST_F(FFTSPlusConfigurationUTEST, GetFftsEnableWithO1Level1) {
  Configuration &config = Configuration::Instance();
  config.runmode_ = "";
  setenv("ASCEND_ENHANCE_ENABLE", "1", 1);
...
TEST_F(FFTSPlusConfigurationUTEST, GetFftsMergeLimitHBMRatio) {
  Configuration &config = Configuration::Instance();
  config.hbm_ratio_ = 0;
