...
TEST(AicpuCustKernelBuilder, CalcOpRunningParam_WorkSpaceInfo_SUCCESS)
{
    map<string, OpsKernelBuilderPtr> opsKernelBuilders;
    GetOpsKernelBuilderObjs(opsKernelBuilders);
    ...
    GeTensorDesc tensor(GeShape(shape), FORMAT_NCHW, DT_FLOAT);
    opDescPtr->AddInputDesc("x", tensor);
    opDescPtr->AddInputDesc("y", tensor);
    opDescPtr->AddOutputDesc("z", tensor);
    ge::AttrUtils::SetBool(opDescPtr, kCustAicpuFlag, true);
    ge::AttrUtils::SetInt(opDescPtr, kWorkspaceSize, 2);
    shared_ptr<ComputeGraph> graphPtr = make_shared<ComputeGraph>("graph");
    ASSERT_EQ(opsKernelBuilders["aicpu_ascend_builder"]->CalcOpRunningParam(*(graphPtr->AddNode(opDescPtr))), SUCCESS);
}
...
TEST(AicpuCustKernelBuilder, NewCustOpp_GenerateTask_SUCCESS)
{
    char *build_path = getenv("BUILD_PATH");
    std::string opp_path = std::string(build_path) + "tests/engines/cpueng/stub/config/vendors/test1";
    char *path = (char *)opp_path.c_str();
