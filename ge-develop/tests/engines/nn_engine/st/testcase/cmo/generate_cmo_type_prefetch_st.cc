...
TEST_F(GenerateCmoTypePrefetchTest, GenerateTypePrefetch) {
  ...
  GeTensorPtr dim_tensor = std::make_shared<GeTensor>(data_tensor_desc, dims_value_vec);
  GeTensorPtr dim_large_tensor = std::make_shared<GeTensor>(data_large_tensor_desc, dims_value_vec);
...
TEST_F(GenerateCmoTypePrefetchTest, GenerateTypeSamePrefetch) {
  ...
  vector<uint8_t> dims_value_vec = {2, 3};
  GeTensorPtr dim_tensor = std::make_shared<GeTensor>(const_tensor_desc, dims_value_vec);
