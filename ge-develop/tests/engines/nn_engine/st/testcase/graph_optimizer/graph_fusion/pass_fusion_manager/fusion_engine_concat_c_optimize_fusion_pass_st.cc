...
class STEST_concat_c_optimize : public testing::Test {
 public:
...
TEST_F(STEST_concat_c_optimize, do_optimize_success_0) {
...
TEST_F(STEST_concat_c_optimize, do_optimize_fail_0) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  InitGraph9(graph);
  ConcatCOptimizeFusionPass pass;
  vector<GraphPass*> passes = {&pass};
...
TEST_F(STEST_concat_c_optimize, check_is_valid_concat) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  InitGraph5(graph);
  ConcatCOptimizeFusionPass pass;
  vector<GraphPass*> passes = {&pass};
...
TEST_F(STEST_concat_c_optimize, check_aligned) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  InitGraph6(graph);
  ConcatCOptimizeFusionPass pass;
...
TEST_F(STEST_concat_c_optimize, do_optimize_success_1) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  OpDescPtr conv1 = std::make_shared<OpDesc>("conv1", CONV2D);
...
TEST_F(STEST_concat_c_optimize, do_optimize_not_changed) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  OpDescPtr conv1 = std::make_shared<OpDesc>("conv1", CONV2D);
  OpDescPtr conv2 = std::make_shared<OpDesc>("conv2", "Conv2DTransposeD");
  OpDescPtr relu1 = std::make_shared<OpDesc>("relu1", "Relu");
...
TEST_F(STEST_concat_c_optimize, check_alignment_fail) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  OpDescPtr conv1 = std::make_shared<OpDesc>("conv1", CONV2D);
  OpDescPtr conv2 = std::make_shared<OpDesc>("conv2", CONV2D);
  OpDescPtr concat = std::make_shared<OpDesc>("concat", CONCATD);
...
TEST_F(STEST_concat_c_optimize, check_alignment_succ) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  OpDescPtr conv1 = std::make_shared<OpDesc>("conv1", CONV2D);
  OpDescPtr conv2 = std::make_shared<OpDesc>("conv2", CONV2D);
  OpDescPtr concat = std::make_shared<OpDesc>("concat", CONCATD);
...
TEST_F(STEST_concat_c_optimize, get_concat_dim_fail_not_changed) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  OpDescPtr conv1 = std::make_shared<OpDesc>("conv1", CONV2D);
  OpDescPtr conv2 = std::make_shared<OpDesc>("conv2", CONV2D);
  OpDescPtr concat = std::make_shared<OpDesc>("concat", CONCATD);
...
TEST_F(STEST_concat_c_optimize, get_concat_dim_negative_succ) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  OpDescPtr conv1 = std::make_shared<OpDesc>("conv1", CONV2D);
  OpDescPtr conv2 = std::make_shared<OpDesc>("conv2", CONV2D);
  OpDescPtr concat = std::make_shared<OpDesc>("concat", CONCATD);
...
TEST_F(STEST_concat_c_optimize, root_graph_unknown_succ) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  InitGraph13(graph);
...
TEST_F(STEST_concat_c_optimize, owner_graph_unknown_no_changed_1) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  InitGraph13(graph);
...
TEST_F(STEST_concat_c_optimize, owner_graph_unknown_no_changed_2) {
  ge::ComputeGraphPtr graph = std::make_shared<ComputeGraph>(GRAPH_NAME);
  InitGraph13(graph);
