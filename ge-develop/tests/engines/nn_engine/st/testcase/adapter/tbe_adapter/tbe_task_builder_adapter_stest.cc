...
TEST_F(STEST_TbeTaskBuilderAdapter, coverage_01) {
  int32_t input_num;
  int32_t output_num;
  uint64_t cur_ptr;
  void *x = nullptr;
  ...
  EXPECT_NE(adapter_->SaveTeCoreL2FlowDataForL2Fusion(input_num, output_num, cur_ptr, xx, yy, tel2ctrl, l2_args_size,
                                                      workspace_num),
            fe::SUCCESS);
}

TEST_F(STEST_TbeTaskBuilderAdapter, coverage_02) {
  int32_t input_num = 1;
...
TEST_F(STEST_TbeTaskBuilderAdapter, init_input) {
...
TEST_F(STEST_TbeTaskBuilderAdapter, InitInputOnEachMode) {
...
TEST_F(STEST_TbeTaskBuilderAdapter, CheckArrayValue) {
  NodePtr node_ptr = CreateNodeWithInputNode(VARIABLE);
  auto tbe_task_builder_adapter =
      shared_ptr<TbeTaskBuilderAdapter>(new (nothrow) TbeTaskBuilderAdapter(*node_ptr, context_));
...
TEST_F(STEST_TbeTaskBuilderAdapter, CheckArrayValue_02) {
  NodePtr node_ptr = CreateNodeWithInputNode(VARIABLE);
  auto tbe_task_builder_adapter =
      shared_ptr<TbeTaskBuilderAdapter>(new (nothrow) TbeTaskBuilderAdapter(*node_ptr, context_));
...
TEST_F(STEST_TbeTaskBuilderAdapter, AppendGlobalData) {
  NodePtr node_ptr = CreateNodeWithInputNode(VARIABLE);
  auto tbe_task_builder_adapter =
      shared_ptr<TbeTaskBuilderAdapter>(new (nothrow) TbeTaskBuilderAdapter(*node_ptr, context_));
  ...
  EXPECT_EQ(device_addrs.size(), 1);
}

TEST_F(STEST_TbeTaskBuilderAdapter, MemCpyForL2IdAndL2Addr_0) {
  NodePtr node_ptr = CreateNodeWithInputNode(VARIABLE);
  auto tbe_task_builder_adapter =
      shared_ptr<TbeTaskBuilderAdapter>(new (nothrow) TbeTaskBuilderAdapter(*node_ptr, context_));
