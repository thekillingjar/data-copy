...
class TBE_ELTWISE_WRITE_SELECT_FUSION_SLICE_INFO_UNITTEST : public testing::Test {
public:
...
protected:
  ...
  void BuildGraphForTbeEltwiseWriteSelectFusionPassOKCase1(ComputeGraphPtr &graph) {
    OpDescPtr data = std::make_shared<OpDesc>("DATA0", fe::DATA);
    OpDescPtr eltwise = std::make_shared<OpDesc>("eltwise", "Eltwise");
    OpDescPtr write_select = std::make_shared<OpDesc>("writeselect", "WriteSelect");
    OpDescPtr end = std::make_shared<OpDesc>("end", "End");
    SetPattern(eltwise, "ElemWise");
    SetPattern(write_select, "write_select");
    SetTvmType(eltwise);
    SetTvmType(write_select);
    ge::AttrUtils::SetInt(eltwise, FE_IMPLY_TYPE, EN_IMPL_HW_TBE);
    ge::AttrUtils::SetInt(write_select, FE_IMPLY_TYPE, EN_IMPL_HW_TBE);
    // add descriptor
    ...
    write_select->AddInputDesc(tensor_desc);
    write_select->AddOutputDesc(tensor_desc);
    end->AddInputDesc(tensor_desc);
    end->AddOutputDesc(tensor_desc);

    NodePtr data_node = graph->AddNode(data);
    NodePtr eltwise_node = graph->AddNode(eltwise);
    NodePtr write_select_node = graph->AddNode(write_select);
    ...
    GraphUtils::AddEdge(eltwise_node->GetOutDataAnchor(0), write_select_node->GetInDataAnchor(0));
    GraphUtils::AddEdge(write_select_node->GetOutDataAnchor(0), end_node->GetInDataAnchor(0));
  }
};

TEST_F(TBE_ELTWISE_WRITE_SELECT_FUSION_SLICE_INFO_UNITTEST, pass_case1) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
  BuildGraphForTbeEltwiseWriteSelectFusionPassOKCase1(graph_out);
  ...
  uint32_t id = 0;
  cerr << endl;
  cerr << "TBE_ELTWISE_WRITE_SELECT_FUSION_SLICE_INFO_UNITTEST::pass_case1 UB fusion before" << endl;
  ...
  int find = 0;
  cerr << endl;
  cerr << "TBE_ELTWISE_WRITE_SELECT_FUSION_SLICE_INFO_UNITTEST::pass_case1 UB fusion result" << endl;
  for (auto &node : graph_out->GetDirectNode()) {
    cerr  << "name: " << node->GetName() << ", type:" << node->GetOpDesc()->GetType() << endl;
    if (node->GetOpDesc()->GetType() == "Eltwise" && node->GetName() == "eltwisewriteselect") {
