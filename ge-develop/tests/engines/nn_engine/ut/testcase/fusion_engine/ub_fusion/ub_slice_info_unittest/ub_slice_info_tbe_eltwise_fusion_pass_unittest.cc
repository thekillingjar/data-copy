 ...
 *************************/
  ...
  //   {"inputList":[{"axis":[1],"headOverLap":[],"idx":0,"tailOverLap":[]}],"outputList":[{"axis":[1],"idx":0}]},
  //   {"inputList":[{"axis":[0],"headOverLap":[],"idx":0,"tailOverLap":[]}],"outputList":[{"axis":[0],"idx":0}]}]}}
  void BuildGraphForTbeEltwiseFusionPassOKCase1(ComputeGraphPtr &graph) {
    ...
    GraphUtils::AddEdge(eltwise2_node->GetOutDataAnchor(0), end_node->GetInDataAnchor(0));
  }
};

TEST_F(TBE_ELTWISE_FUSION_SLICE_INFO_UNITTEST, pass_case1) {
  ComputeGraphPtr graph_out = std::make_shared<ComputeGraph>("test");
  BuildGraphForTbeEltwiseFusionPassOKCase1(graph_out);
