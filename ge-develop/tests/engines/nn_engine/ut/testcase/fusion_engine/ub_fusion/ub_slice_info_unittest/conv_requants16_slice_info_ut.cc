...
class CONV_REQUANTS16_SLICE_INFO_UTEST : public testing::Test {
 public:
 protected:
  virtual void SetUp() {
    conv_requant_s16_slice_info_ptr = std::make_shared<ConvRequantS16SliceInfo>();
  }
 ...
 public:
  std::shared_ptr<ConvRequantS16SliceInfo> conv_requant_s16_slice_info_ptr;
};


TEST_F(CONV_REQUANTS16_SLICE_INFO_UTEST, modify_slice_info_by_pattern_suc) {
  ...
  size_t input_size = 1;
  bool is_head_fusion;
  Status ret = conv_requant_s16_slice_info_ptr->ModifySliceInfoByPattern(fusion_node, fusion_nodes, op_calc_info,
                                                                         input_size, is_head_fusion);
  EXPECT_EQ(fe::SUCCESS, ret);
}

TEST_F(CONV_REQUANTS16_SLICE_INFO_UTEST, set_output_slice_info_for_requants16_suc) {
  ComputeGraphPtr graph = std::make_shared<ge::ComputeGraph>("test");
  BuildGraph_1(graph);
  ge::NodePtr fusion_node = graph->FindNode("dequant");
  OpCalcInfo op_calc_info;
  ...
  Status ret = conv_requant_s16_slice_info_ptr->SetOutputSliceInfoForRequantS16(op_calc_info);
  EXPECT_EQ(fe::SUCCESS, ret);
}
