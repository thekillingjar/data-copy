...
HcclResult hcom_broadcast(const char *tag, void *ptr, u64 count, HcclDataType data_type, u32 root, const char *group,
                          rtStream_t stream) {
  return HCCL_SUCCESS;
}
...
HcclResult hcom_reduce_scatter(const char *tag, void *input_ptr, void *output_ptr, u64 count, HcclDataType data_type,
                               HcclReduceOp op, const char *group, rtStream_t stream) {
  return HCCL_SUCCESS;
}
