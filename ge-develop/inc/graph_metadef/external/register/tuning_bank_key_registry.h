...
#define DECLARE_STRUCT_RELATE_WITH_OP(op, bank_key, ...)                                                               \
  ...
  static bool ParseFunc##op##bank_key(const std::shared_ptr<void> &in_args, size_t len, ge::AscendString &bank_key_str) {      \
    if (sizeof(bank_key_str) != len || in_args == nullptr) {                                                               \
      return false;                                                                                                    \
    }                                                                                                                  \
    return false;                                                                                                       \
  }                                                                                                                    \
  static bool LoadFunc##op##bank_key(std::shared_ptr<void> &in_args, size_t &len, const ge::AscendString &bank_key_str) {      \
