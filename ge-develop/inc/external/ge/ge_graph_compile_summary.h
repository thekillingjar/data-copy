...
#include "ge_external_weight_desc.h"
...
public:
  ...
  /**
   * @brief get root graph and subgraph's AscendString of the stream graph in DOT file format
  ...
  /**
   * @brief get root graph and subgraph's logical stream infos
...
class GE_FUNC_VISIBILITY LogicalStreamAllocationInfo {
 public:
  ...
  /**
   * @brief get physical stream num of current logical stream
...
/**
 * @brief get root graph and subgraph's logical stream ids
 * @param [in] compiled_graph_summary
...
/**
 * @brief get root graph and subgraph's user stream label
 * @param [in] compiled_graph_summary
...
/**
 * @brief get root graph and subgraph's physical stream nums
 * @param [in] compiled_graph_summary
 * @param [out] map, key is graph name, value is vector of physical stream numswhich index is logical stream id
 * @return result of function
 */
...
/**
 * @brief get root graph and subgraph's hccl followed stream nums
 * @param [in] compiled_graph_summary
 *        [out] map, key is graph name, value is vector of hccl followed stream nums which index is logical stream id
 * @return result of function
 */
...
/**
 * @brief get root graph and subgraph's all nodes
 * @param [in] compiled_graph_summary
 * @param [out] map, key is graph name, value is vector of all nodes which index is logical stream id
 * @return result of function
 */
ge::Status GEStreamAllocationSummaryGetAllNodes(
    const ge::CompiledGraphSummary &compiled_graph_summary,
    std::map<AscendString, std::vector<std::vector<GNode>>> &graph_to_all_nodes);

/**
 * @brief get root graph and subgraph's stream graph
 * @param [in] compiled_graph_summary
 * @param [out] map, key is graph name, value is string of stream graph
 * @return result of function
 */
ge::Status GEStreamAllocationSummaryGetStreamGraphs(const ge::CompiledGraphSummary &compiled_graph_summary,
