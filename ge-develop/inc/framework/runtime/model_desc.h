...
class VISIBILITY_EXPORT ModelDesc {
 public:
  ...
  void SetSpaceRegistries(OpImplSpaceRegistryV2Array *space_registries);
  OpImplSpaceRegistryV2Array *GetSpaceRegistries() const;
  ...
  size_t GetReusableNotifyNum() const;
  void SetReusableNotifyNum(const size_t notify_num);

  size_t GetAttachedStreamNum() const;
  void SetAttachedStreamNum(const size_t stream_num);
 ...
 private:
  ...
  size_t reusable_notify_num_;
  size_t attached_stream_num_;
