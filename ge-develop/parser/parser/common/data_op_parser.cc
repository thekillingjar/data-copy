...
FMK_FUNC_HOST_VISIBILITY Status DataOpParser::ParseShape(const std::vector<int64_t> &shape, ge::OpDescPtr op) {
  ...
  i_tensor_desc.SetFormat(ge::TypeUtilsInner::DomiFormatToFormat(GetParserContext().format));
  i_tensor_desc.SetOriginFormat(ge::TypeUtilsInner::DomiFormatToFormat(GetParserContext().format));
  o_tensor_desc.SetFormat(ge::TypeUtilsInner::DomiFormatToFormat(GetParserContext().format));
  if (op->AddInputDesc(i_tensor_desc) != ge::GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "AddInputDesc failed for op %s.", op->GetName().c_str());
  ...
  if (op->AddOutputDesc(o_tensor_desc) != ge::GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "AddOutputDesc failed for op %s.", op->GetName().c_str());
...
Status DataOpParser::Init5DInputTensor(const std::vector<int64_t> &shape, ge::GeTensorDesc &tensor_desc) {
  ...
  if (graph_status != ge::GRAPH_SUCCESS) {
    REPORT_INNER_ERR_MSG("E19999", "GetTensorSizeInBytes failed");
