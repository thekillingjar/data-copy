...
#ifndef AIR_RUNTIME_COMMON_MEMORY_STATISTIC_MANAGER_H
#define AIR_RUNTIME_COMMON_MEMORY_STATISTIC_MANAGER_H
#include <cstdint>
#include <mutex>
#include <thread>
#include <string>
...
class MemoryStatisticManager {
 public:
  ...
  void Initialize(const std::string &xsmem_group_name);
  void Finalize();
 ...
 private:
  ...
  // used for notify thread exit immediately
  std::mutex mtx_;
...
#endif  // AIR_RUNTIME_COMMON_MEMORY_STATISTIC_MANAGER_H
