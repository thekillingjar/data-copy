...
struct NanoZeroCopyValueToKey {
  int64_t offset_;
  explicit NanoZeroCopyValueToKey(int64_t offset) : offset_(offset) {}
  bool operator()(const std::pair<int64_t, uint32_t> &item) const {
    return item.second == static_cast<uint32_t>(offset_);
  }
};
...
class NanoDbgData {
 ...
 private:
  ...
  void SaveMemType(std::map<int64_t, std::vector<int32_t>> &mem_types, const int64_t id,
  ...
  std::map<int64_t, std::vector<int32_t>> output_mem_types_;
  std::map<int64_t, std::vector<int32_t>> input_mem_types_;
};
}  // namespace ge
