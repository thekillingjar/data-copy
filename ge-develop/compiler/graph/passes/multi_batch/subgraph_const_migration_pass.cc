...
const std::string kMbatchNodeNameMark = "_ascend_mbatch_batch_";
...
bool IsSameConstNode(const NodePtr &src_node, const NodePtr &dst_node) {
  ...
  if ((src_node->GetType() != CONSTANT) || (dst_node->GetType() != CONSTANT)) {
    return false;
  }
  ...
  GeTensorPtr src_weight;
  GeTensorPtr dst_weight;
  // empty const
  if (!AttrUtils::MutableTensor(src_node->GetOpDesc(), ATTR_NAME_WEIGHTS, src_weight) &&
      !AttrUtils::MutableTensor(dst_node->GetOpDesc(), ATTR_NAME_WEIGHTS, dst_weight)) {
    return true;
  }
  if (!AttrUtils::MutableTensor(src_node->GetOpDesc(), ATTR_NAME_WEIGHTS, src_weight) ||
      !AttrUtils::MutableTensor(dst_node->GetOpDesc(), ATTR_NAME_WEIGHTS, dst_weight)) {
  ...
  proto::TensorDef src_attr_def;
  proto::TensorDef dst_attr_def;
  GeTensorSerializeUtils::GeTensorAsProto(*src_weight, &src_attr_def);
  GeTensorSerializeUtils::GeTensorAsProto(*dst_weight, &dst_attr_def);
  // value equal
  return src_attr_def.data() == dst_attr_def.data();
}
}  // namespace
...
Status SubgraphConstMigrationPass::Run(ComputeGraphPtr graph) {
  ...
  (void)AttrUtils::GetBool(graph, ATTR_NAME_SUBGRAPH_IS_MULTI_DIMS, is_subgraph_dyn_dims);

  if ((graph->GetParentGraph() != nullptr) && (!is_subgraph_dyn_dims)) {
    GELOGD("Subgraph %s has parent graph, skip the SubgraphConstMigrationPass", graph->GetName().c_str());
    return SUCCESS;
  }

  GELOGD("Begin to run Subgraph Const Migration pass on graph: %s, is_subgraph_dyn_dims is %d.",
         graph->GetName().c_str(), static_cast<int32_t>(is_subgraph_dyn_dims));
  for (const auto &node : graph->GetDirectNode()) {
    ...
    if (!func_desc->HasAttr(ATTR_NAME_BATCH_NUM)) {
      GELOGD("Not multi-batch, skip case, node name: %s", node->GetName().c_str());
      continue;
    }
    ...
    const auto &const_nodes = all_const_nodes.cbegin()->second;
    for (const auto &item : const_nodes) {
      if (GraphNodeMigration(graph, node, all_const_nodes, all_data_nodes, item) != SUCCESS) {
        return FAILED;
      }
    }
  }
...
Status SubgraphConstMigrationPass::ClassifyGraphNodes(const ComputeGraphPtr &graph, const OpDescPtr &func_desc,
  ...
  for (const auto &name : func_desc->GetSubgraphInstanceNames()) {
    ...
    std::set<NodePtr> ctrl_only_const_nodes;
    ...
    for (auto &node : subgraph->GetDirectNode()) {
      if (node->GetType() == DATA) {
        ...
        std::set<std::string> peer_name_list;
        ...
        std::string key_of_const;
        for (const std::string &peer_name : peer_name_list) {
          key_of_const += (key_of_const.empty() ? peer_name : "_" + peer_name);
        }
...
void SubgraphConstMigrationPass::GetPeerNameList(const NodePtr &node, std::set<std::string> &peer_name_list) const {
  ...
  for (const auto &in_anchor : out_anchor->GetPeerInDataAnchors()) {
    ...
    std::string fixed_name = node_full_name.substr(0, pos);
...
///
bool SubgraphConstMigrationPass::GetAssociatedNodes(
...
///
bool SubgraphConstMigrationPass::IsParallelNodeSame(
...
/// @return 0: SUCCESS / others: FAILED
///
Status SubgraphConstMigrationPass::GraphNodeMigration(
    const ComputeGraphPtr &graph, const NodePtr &func_node,
    const OrderedGraphToConstNodes &all_const_nodes,
    OrderedGraphToDataNodes &all_data_nodes,
    const std::pair<std::string, NodePtr> const_node) const {
  if (!IsParallelNodeSame(all_const_nodes, const_node.second, const_node.first)) {
    return SUCCESS;
  }

  // Get associated Data, if Data feed other nodes, need append new Data.
  uint32_t parent_index = kInvalidParent;
  if (!GetAssociatedNodes(all_data_nodes, const_node.second, parent_index)) {
    return SUCCESS;
  }

  GELOGI("Move node: %s, parent index: %u", const_node.second->GetName().c_str(), parent_index);
  if (AppendParallelNode(func_node, parent_index, all_data_nodes) != SUCCESS) {
    return FAILED;
  }

  if (MoveNodeToParent(graph, func_node, all_const_nodes, all_data_nodes, const_node.first, parent_index) != SUCCESS) {
    return FAILED;
  }
...
/// @return 0: SUCCESS / others: FAILED
///
Status SubgraphConstMigrationPass::AppendParallelNode(
  ...
  for (auto &item : all_data_nodes) {
    ...
    const auto data_name = subgraph->GetName() + "_data_subgraph_migration_" + std::to_string(parent_index);
...
Status SubgraphConstMigrationPass::DetachParallelNode(const ComputeGraphPtr &graph, const NodePtr &const_node,
  ...
  GE_CHECK_NOTNULL(ctrl_anchor);
  const auto ctrl_anchors = ctrl_anchor->GetPeerInControlAnchors();
  for (const auto &ctrl_in_anchor : ctrl_anchors) {
    GE_CHK_GRAPH_STATUS_RET(GraphUtils::RemoveEdge(ctrl_anchor, ctrl_in_anchor),
                            "[Remove][ControlEdge] between %s and %s failed",
                            const_node->GetName().c_str(), ctrl_in_anchor->GetOwnerNode()->GetName().c_str());
    GELOGI("Remove Edge: %s %s", const_node->GetName().c_str(), ctrl_in_anchor->GetOwnerNode()->GetName().c_str());

    GE_CHK_GRAPH_STATUS_RET(GraphUtils::AddEdge(data_node->GetOutControlAnchor(), ctrl_in_anchor),
                            "[Add][ControlEdge] between %s and %s failed",
                            data_node->GetName().c_str(), ctrl_in_anchor->GetOwnerNode()->GetName().c_str());
    GELOGI("Add Edge: %s %s", data_node->GetName().c_str(), ctrl_in_anchor->GetOwnerNode()->GetName().c_str());
  }
  ...
  for (const auto &data_in_anchor : in_anchors) {
    GE_CHK_GRAPH_STATUS_RET(GraphUtils::RemoveEdge(out_anchor, data_in_anchor),
                            "[Remove][Edge] between %s and %s failed",
                            const_node->GetName().c_str(), data_in_anchor->GetOwnerNode()->GetName().c_str());
    GELOGI("Remove Edge: %s %s", const_node->GetName().c_str(), data_in_anchor->GetOwnerNode()->GetName().c_str());

    GE_CHK_GRAPH_STATUS_RET(GraphUtils::AddEdge(data_node->GetOutDataAnchor(kZeroIndex), data_in_anchor),
                            "[Add][Edge] between %s and %s failed",
                            data_node->GetName().c_str(), data_in_anchor->GetOwnerNode()->GetName().c_str());
    GELOGI("Add Edge: %s %s", data_node->GetName().c_str(), data_in_anchor->GetOwnerNode()->GetName().c_str());
  }
...
/// @return 0: SUCCESS / others: FAILED
///
Status SubgraphConstMigrationPass::MoveNodeToParent(
    const ComputeGraphPtr &graph, const NodePtr &func_node,
    const OrderedGraphToConstNodes &all_const_nodes,
    const OrderedGraphToDataNodes &all_data_nodes,
    const std::string &node_key, uint32_t parent_index) const {
  if (node_key.empty() || parent_index == kInvalidParent) {
    REPORT_INNER_ERR_MSG("E19999", "Param node_key is empty or param parent_index is 0x%X, check invalid",
                       kInvalidParent);
  ...
  for (auto &item : all_const_nodes) {
    ...
    if (it_const == item.second.end()) {
      REPORT_INNER_ERR_MSG("E19999", "Const node name:%s not found in graph:%s, check invalid",
...
REG_PASS_OPTION("SubgraphConstMigrationPass").LEVELS(OoLevel::kO1);
}  // namespace ge
