...
class MultiBatchPass : public GraphPass {
 ...
 private:
  ...
  Status SetCaseLabel(const ComputeGraphPtr &graph, const NodePtr &case_node) const;
  static void GetAllGenMaskNodes(const ComputeGraphPtr &graph,
                                 std::unordered_map<ComputeGraphPtr, std::vector<NodePtr>> &subgraph_to_gen_mask_nodes);
  static Status TryToReplaceConstInput(const ComputeGraphPtr &graph, const std::vector<NodePtr> &gen_mask_nodes);
  static Status PreparingForGenMaskParallel(const ComputeGraphPtr &graph);
};
}  // namespace ge
