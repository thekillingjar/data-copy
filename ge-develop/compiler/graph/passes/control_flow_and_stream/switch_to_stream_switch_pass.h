...
class SwitchToStreamSwitchPass : public GraphPass {
 ...
 private:
  ...
  /// @param [out] peer_cond_anchor
  /// @return Status
  Status FindSwitchCondInput(OutDataAnchorPtr &peer_cond_anchor) const;
  ...
  /// @param [in] node
  /// @return group_id
  int64_t GetGroupId(const NodePtr &node) const;
  ...
  /// @return Status
  Status AddConstNode(const ComputeGraphPtr &graph, const NodePtr &stream_switch) const;
  ...
  /// @param [in] graph
  /// @return Status
  Status UpdateControlFlowGroup(const ComputeGraphPtr &graph);
  ...
  std::map<int64_t, int64_t> old_2_new_group_index_;
};
}  // namespace ge
