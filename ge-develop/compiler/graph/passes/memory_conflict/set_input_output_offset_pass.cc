...
Status SetInputOutputOffsetPass::Run(ComputeGraphPtr graph) {
  ...
  if ((parent_node != nullptr) && parent_node->GetOpDescBarePtr()->HasAttr(ATTR_NAME_FFTS_PLUS_SUB_GRAPH)) {
    return SUCCESS;
  }
  auto ffts_filter = [](const Node &node, const char_t *, const ComputeGraphPtr &) {
    return !node.GetOpDescBarePtr()->HasAttr(ATTR_NAME_FFTS_PLUS_SUB_GRAPH);
  };
  for (auto &node : graph->GetNodes(false, nullptr, ffts_filter)) {
    GE_CHECK_NOTNULL(node->GetOpDesc());
    std::vector<int32_t> connect_input;
    ...
    std::vector<int32_t> connect_output;
...
Status SetInputOutputOffsetPass::SetInputOffsetForFusion(const std::vector<int64_t> &memory_type,
  ...
  for (size_t i = 0; i < memory_type.size(); ++i) {
    if ((memory_type.at(i) != RT_MEMORY_L1) && (memory_type.at(i) != static_cast<int64_t>(kRtMemoryUB))) {
      std::vector<int64_t> input_offset_of_node = op_desc->GetInputOffset();
      if (input_offset_of_node.size() < i) {
        REPORT_INNER_ERR_MSG("E19999", "Input offsets size:%zu of node:%s(%s) < index:%zu, check invalid",
      ...
      int32_t out_index = peer_out_anchor->GetIdx();
      auto data_node = NodeUtils::GetInNodeCrossSubgraph(peer_out_anchor->GetOwnerNode());
...
Status SetInputOutputOffsetPass::SetInputOffsetForHcom(const NodePtr &node,
    const std::vector<int32_t> &connect_input) const {
  ...
  for (size_t input_index = 0; input_index < connect_input.size(); ++input_index) {
    int32_t connect_input_index = connect_input.at(input_index);
    ...
    if (in_op_desc->GetType() == DATA) {
      ...
      if (output_offset == input_offset) {
        ...
        GELOGI("input offset from %s to %s is %ld to %ld.", in_op_desc->GetName().c_str(), node->GetName().c_str(),
...
Status SetInputOutputOffsetPass::SetInputOffset(const NodePtr &node, const std::vector<int32_t> &connect_input) const {
  GELOGD("Start to set InputOffset for [%s].", node->GetName().c_str());
...
Status SetInputOutputOffsetPass::SetOutputOffsetForConcat(const NodePtr &node) const {
  ...
  GE_IF_BOOL_EXEC(output_offset_of_concat.size() != 1,
                  REPORT_INNER_ERR_MSG("E19999", "Output offsets size:%zu of node:%s(%s) not equal to 1, check invalid",
  ...
  // PC -> A
  // pc -> netoutput
  // if netoutput is not first out node,need to find
  NodePtr net_output = node->GetOutDataNodes().at(0);
  for (const auto &out_node_direct : node->GetOutDataNodes()) {
    if (out_node_direct->GetType() == NETOUTPUT) {
      net_output = out_node_direct;
      break;
    }
  }
  // pc->netoutput->netoutput
  auto parent_node = node->GetOwnerComputeGraph()->GetParentNode();
  if (parent_node != nullptr) {
    for (const auto &out_node : parent_node->GetOutDataNodes()) {
      if (out_node->GetType() != NETOUTPUT) {
        net_output = node->GetOutDataNodes().at(0);
        break;
      }
      net_output = out_node;
    }
  ...
  GELOGI("update offset to node %s, basic offset size %zu, relative offset %zu",
      out_op_desc->GetName().c_str(), zero_copy_basic_offset.size(), zero_copy_relative_offset.size());
  GE_CHK_BOOL_EXEC(ge::AttrUtils::SetListInt(out_op_desc, ATTR_ZERO_COPY_BASIC_OFFSET, zero_copy_basic_offset),
                   REPORT_INNER_ERR_MSG("E19999", "Set Attr:%s to op:%s(%s) failed", ATTR_ZERO_COPY_BASIC_OFFSET.c_str(),
...
Status SetInputOutputOffsetPass::SetOutputOffsetForHcom(const NodePtr &node,
                                                        const std::vector<int32_t> &connect_output) const {
  GELOGI("Start SetOutputOffsetForHcom, %s connect with %zu output.", node->GetName().c_str(), connect_output.size());
  std::vector<int64_t> output_offset_of_node;
  output_offset_of_node = node->GetOpDesc()->GetOutputOffset();
  int32_t connect_output_index = connect_output.at(0);
  GE_ASSERT_TRUE(static_cast<size_t>(connect_output_index) < output_offset_of_node.size(),
                 "connect_output_index: %d, output_offset_of_node size: %zu",
                 connect_output_index, output_offset_of_node.size());
  ...
  const auto &out_data_anchor = node->GetOutDataAnchor(connect_output_index);
  GE_ASSERT_NOTNULL(out_data_anchor, "node[%s] output index[%d]", node->GetNamePtr(), connect_output_index);
  ...
  for (const auto in_anchor : out_data_anchor->GetPeerInDataAnchorsPtr()) {
    ...
    if (in_anchor->GetOwnerNodeBarePtr()->GetType() == NETOUTPUT) {
      net_output = in_anchor->GetOwnerNode();
      break;
    }
  }
  if (net_output == nullptr) {
    GELOGI("node[%s] output[%d] not connect netoutput", node->GetNamePtr(), connect_output_index);
    return SUCCESS;
  }
  auto out_op_desc = net_output->GetOpDesc();
  GE_CHECK_NOTNULL(out_op_desc);
...
Status SetInputOutputOffsetPass::SetOutputOffset(const NodePtr &node,
                                                 const std::vector<int32_t> &connect_output) const {
  GELOGD("Start to set OutputOffset for [%s].", node->GetName().c_str());
...
bool SetInputOutputOffsetPass::CheckBufferFusion(const NodePtr &node) const {
...
REG_PASS_OPTION("SetInputOutputOffsetPass").LEVELS(OoLevel::kO0);
}  // namespace ge
