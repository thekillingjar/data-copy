...
namespace ge {
Status UserInputAndNotRtsSpecailOutputChecker(CheckFuncContext &context) {
  bool done = false;
  ...
  auto user_in_node_index_io = context.node_b;
  if (MemLayoutConflictUtil::IsContainTargetType(context.type_a, ANCHOR_ATTR_USER_MEMORY_INPUT)) {
    user_in_node_index_io = context.node_a;
  }
...
REGISTER_FUNC(ANCHOR_ATTR_USER_MEMORY_INPUT, ANCHOR_ATTR_RTS_SPECIAL_TYPE_OUTPUT,
              UserInputAndNotRtsSpecailOutputChecker);
}  // namespace ge
