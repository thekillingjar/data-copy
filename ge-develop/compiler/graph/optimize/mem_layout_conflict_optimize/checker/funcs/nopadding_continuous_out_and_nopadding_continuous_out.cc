...
namespace ge {
Status NoPaddingContinuousOutputAndNoPaddingContinuousOutputChecker(CheckFuncContext &context) {
  std::set<NodePtr> nodes_set;
  std::set<NodePtr> nopadding_continuous_nodes;
  for (const auto &node_index_io : context.all_nodes) {
    if (nodes_set.insert(node_index_io.node_).second) {
      if (MemLayoutConflictUtil::IsNoPaddingContinuousOutput(node_index_io.node_)) {
        nopadding_continuous_nodes.insert(node_index_io.node_);
      }
    }
  }
  for (const auto &node : nopadding_continuous_nodes) {
