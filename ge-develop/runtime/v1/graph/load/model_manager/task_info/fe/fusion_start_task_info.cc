...
Status FusionStartTaskInfo::Init(const domi::TaskDef &task_def, DavinciModel *const davinci_model,
  ...
  GELOGI("FusionStartTaskInfo Init Success, logic stream id: %u, stream: %p.", task_def.stream_id(), stream_);
  return SUCCESS;
}
...
Status FusionStartTaskInfo::Distribute() {
  ...
  if (rt_ret != RT_ERROR_NONE) {
    REPORT_INNER_ERR_MSG("E19999", "Call rtKernelFusionStart failed, ret:%d", rt_ret);
    GELOGE(RT_FAILED, "[Call][RtKernelFusionStart] failed, ret:%d", rt_ret);
    return RT_ERROR_TO_GE_STATUS(rt_ret);
  }
  is_support_redistribute_ = true;
  GELOGI("FusionStartTaskInfo Distribute Success, stream: %p.", stream_);
  return SUCCESS;
}

REGISTER_TASK_INFO(MODEL_TASK_FUSION_START, FusionStartTaskInfo);
}  // namespace ge
